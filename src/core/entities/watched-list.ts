export abstract class WatchedList<T> {
  ...
  private isNewItem(item: T): boolean {
    return this.new.some((i) => this.compareItems(i, item))
  }

  private isRemovedItem(item: T): boolean {
    return this.removed.some((i) => this.compareItems(i, item))
  }

  private removeFromNew(item: T): void {
    this.new = this.new.filter((i) => !this.compareItems(i, item))
  }
  ...
  private removeFromRemoved(item: T): void {
    this.removed = this.removed.filter((i) => !this.compareItems(i, item))
  }

  private wasAddedInitially(item: T): boolean {
    return this.initial.some((i) => this.compareItems(i, item))
  }
